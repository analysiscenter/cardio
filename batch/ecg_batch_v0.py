...
class EcgBatch(Batch):
    ...
    def post_parallel_segment(self, all_results, *args, **kwargs):
        ...
        meta = {i: {'__pos': i, 'diag': labels[i], 'origin': origins[i]} for i in range(len(labels))}
    ...
    @action
    @inbatch_parallel(init="init_parallel", post="post_parallel_segment", target='mpc')
    ...
    def fft_inception():
        def step_decay(epoch):
            drop_t, lr = [[0, 50, 100], [0.01, 0.001, 0.0001]]
        ...
        crop_1 = Lambda(crop, arguments={'a': 0, 'b': int(shape_fft[1] / 3)})(fft_1)
        ...
        return model, lrate, hist, diag_code
    ...
    def calc_loss(self, model_comp):
        ...
        print('loss:', log_loss(testY, pred))
    ...
    def show_loss(self, model_comp):
        model, lrate, hist, code = model_comp
        fig = plt.figure()
        plt.plot(hist[0].history["loss"], "r", label="train loss")
        #plt.plot(hist[0].history["val_loss"], "b", label="validation loss")
        plt.legend()
